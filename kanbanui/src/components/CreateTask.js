...
function CreateTask({editTask, createTask, onClose, task, fetchTasks}) {
  ...
  const addTask = async () =>{
    ...
    if(msg === null){
    let res = await fetch("https://localhost:8000/api/task/add",
    {
      method: 'POST',
      headers: {
...
const saveTask = async () =>{
  ...
  if(msg === null){
  let id = task.id;
  let res = await fetch(`https://localhost:8000/api/task/${id}`,
  {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
...
const getAllUsers = async () =>{
  let res = await fetch("https://localhost:8000/api_user/user/")
      res= await res.json()
      setUsers(res);
}
